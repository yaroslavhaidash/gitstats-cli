...
function scheduleInstalled() {
    const os = platform();
    if (os === "darwin")
        return existsSync(PLIST) || existsSync(LEGACY_PLIST);
    if (os === "win32")
        return [JOB, "gitstats-sync"].some((t) => spawnSync("schtasks", ["/Query", "/TN", t], { stdio: "ignore" }).status === 0);
